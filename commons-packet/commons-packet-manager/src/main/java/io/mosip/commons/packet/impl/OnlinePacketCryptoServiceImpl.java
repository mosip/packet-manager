...
public class OnlinePacketCryptoServiceImpl implements IPacketCryptoService {
	...
	public byte[] sign(byte[] packet) {
		try {
			...
			request.setRequesttime(getCurrentUtcDateTime());
			...
			ResponseEntity<String> response = restTemplate.exchange(keymanagerCsSignUrl, HttpMethod.POST, httpEntity,
					String.class);
			LinkedHashMap responseMap = (LinkedHashMap) mapper.readValue(response.getBody(), LinkedHashMap.class)
					.get("response");
	...
	public byte[] encrypt(String refId, byte[] packet) {
		try {
			...
			cryptomanagerRequestDto.setData(CryptoUtil.encodeToURLSafeBase64(packet));
			...
			if (responseObject != null && responseObject.getResponse().getData() != null) {
				...
				byte[] encryptedPacket = EncryptionUtil.mergeEncryptedData(encryptedData, nonce, aad);
				LOGGER.info(PacketManagerLogger.SESSIONID, PacketManagerLogger.REFERENCEID, refId,
						"Successfully encrypted Packet");
				return encryptedPacket;
			}
			throw new PacketDecryptionFailureException("Packet encrypted failed");
		} catch (IOException e) {
			LOGGER.error(PacketManagerLogger.SESSIONID, PacketManagerLogger.REFERENCEID, refId,
					ExceptionUtils.getStackTrace(e));
	...
	public byte[] decrypt(String refId, byte[] packet) {
		try {
			...
			byte[] encryptedData = Arrays.copyOfRange(packet,
					CryptomanagerConstant.GCM_NONCE_LENGTH + CryptomanagerConstant.GCM_AAD_LENGTH, packet.length);
			...
			HttpEntity<RequestWrapper<CryptomanagerRequestDto>> httpEntity = new HttpEntity<>(request);
			ResponseEntity<String> response = restTemplate.exchange(cryptomanagerDecryptUrl, HttpMethod.POST,
					httpEntity, String.class);
			...
			if (responseObject != null && responseObject.getResponse().getData() != null) {
				byte[] decryptedPacket = CryptoUtil.decodeURLSafeBase64(responseObject.getResponse().getData());
				LOGGER.info(PacketManagerLogger.SESSIONID, PacketManagerLogger.REFERENCEID, refId,
						"Successfully decrypted Packet");
				return decryptedPacket;
			}
			throw new PacketDecryptionFailureException("Packet decrypted failed");
		} catch (IOException e) {
			LOGGER.error(PacketManagerLogger.SESSIONID, PacketManagerLogger.REFERENCEID, refId,
					ExceptionUtils.getStackTrace(e));
			throw new PacketDecryptionFailureException(IO_EXCEPTION, e);
	...
	public boolean verify(String refId, byte[] packet, byte[] signature) {
		try {
			...
			ResponseEntity<String> response = restTemplate.exchange(keymanagerCsverifysignUrl, HttpMethod.POST,
					httpEntity, String.class);

			LinkedHashMap responseMap = (LinkedHashMap) mapper.readValue(response.getBody(), LinkedHashMap.class)
					.get("response");// .get("signature");
			if (responseMap != null && responseMap.size() > 0) {
				boolean result = responseMap.get("verified") != null
						&& responseMap.get("verified").toString().equalsIgnoreCase("true");
				if (!result)
					LOGGER.error(PacketManagerLogger.SESSIONID, PacketManagerLogger.REFERENCEID, refId,
	...
	private String getPublicKey(String refId) throws IOException {
		String machineId = refId.split(SPLIT_UNDERSCORE)[1];
		...
		throw new SignatureException("Failed to retrieve TPM public key");
	}

	private LocalDateTime getCurrentUtcDateTime() {
		return LocalDateTime.parse(DateUtils.getUTCCurrentDateTimeString(DATETIME_PATTERN),
